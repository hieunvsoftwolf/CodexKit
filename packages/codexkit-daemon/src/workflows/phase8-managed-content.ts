...
function renderCodexkitConfig(): string {
  return [
    ...
    "managed_by = \"codexkit\"",
    "",
    "# Optional Phase 10 runner contract override",
    "# [runner]",
    "# command = \"codex exec\"",
    ""
  ].join("\n");
...
function renderReadme(repoName: string): string {
  return [
    ...
    "",
    "## Install CodexKit CLI",
    "",
    "Canonical npx-first commands:",
    "",
    ...
    "- `cdx doctor`",
    "",
    "If raw npx fails with ~/.npm ownership EPERM on your host, use:",
    "",
    ...
    "",
    "1. Run `cdx init`, then `cdx init --apply --approve-protected --approve-managed-overwrite`.",
    "2. Run `cdx doctor`.",
    "3. If this repository is install-only, create the first commit before worker-backed workflows.",
    "4. Run `cdx brainstorm <task>`, then `cdx plan <task>`, then `cdx cook <absolute-plan-path>`.",
    "",
    "## Wrapped Runner Example",
    "",
    "- One shell override: `CODEXKIT_RUNNER=\"codex-safe exec --profile beta\" cdx doctor`",
    "- Persisted config: set `.codexkit/config.toml` `[runner] command = \"codex-safe exec --profile beta\"`",
    "",
    "Use `cdx update` for managed content refreshes.",
    ""
  ].join("\n");
}
